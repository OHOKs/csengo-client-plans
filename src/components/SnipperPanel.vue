...
const replace_url = async (input) => {
  const regex =
    "https://(?:m.youtube.com|yt.be|youtube.com|youtu.be|www.youtube.com)";
  if (input.target.value.match(regex)) {
    player.loadVideoByUrl(transformUrl(input.target.value));
    url.value = input.target.value;
  } else {
    triggerToast(
      "This link is not supported, only yout.be youtube.com are valid",
      ToastEnum.Warning
    );
  }
};
...
function transformUrl(url: string): string {
  ...
  if (shortUrlMatch) {
    ...
    const standardUrlMatch = url.match(
...
<template>
  ...
  <input
    ...
    type="text"
    name="url"
    placeholder="Youtube link ide"
  />
  <input
    v-model="title"
    type="text"
    name="url"
    placeholder="Adj neki egy nevet"
  />
  <v-range-slider
    :max="length"
    @end="slider_change"
    step="1"
    :isDisabled="length == 0 ? true : false"
    strict="true"
    v-model="from_to"
    thumb-label="always"
  />
  ...
  <div class="button-container" :class="{ upload: uploading }">
    <button @click="uploadSong" :disabled="uploading">
      Feltöltés YouTube-ról
      <span></span>
      <span></span>
      <span></span>
      <span></span>
